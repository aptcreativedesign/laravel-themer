...
### Compile all themes and also a master mix as well.
The mix-manifest.json used by your application will remain in the root of the public directory and not in the individual theme folders within the public directory.

All theme specific webpack.mix files generated by this package become redundant using this implementation

To compile the shared (root) compiled assets build as normal in the webpack file
````js
mix.js('resources/js/app.js', '/js')
    .js('resources/js/web/klaviyo.js', '/js/web')
    .sass('resources/sass/app.scss', '/css')
    .sourceMaps();
````
To recursively discover and build all template assets paste the below into your webpack file
````js
const fs = require('fs');
const path = require('path');

// function to loop through a directory and retrieve the paths of all the folders within
...
// declare theme src path and an array to populate with the names of all the existing themes
let themesSrcPath = `${__dirname}/themes/`,
    themeNames = [];

// get and set all theme names to the themeNames array
themeDirectoryPaths = getDirectories(themesSrcPath)
themeDirectoryPaths.forEach(path => themeNames.push(/[^/]*$/.exec(path)[0]))

// build all the assets for all of the themes
themeNames.forEach(function (themeName) {       // require(`${__dirname}/themes/${themeName}/webpack.mix.js`)
    mix.js("themes/" + themeName + "/js/app.js", "themes/" + themeName + "/js/app.js")
        .sass("themes/" + themeName + "/sass/app.scss", "themes/" + themeName + "/css/app.css");
